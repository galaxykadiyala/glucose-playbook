...
function PriorityBadge({ priority }) {
  const map = {
    ...
    informational: { bg: '#F5F3FF', text: '#6B21A8', label: 'Info' },
  }
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={160}>
        <LineChart data={chartData} margin={{ top: 4, right: 8, bottom: 0, left: -20 }}>
          ...
          <Line type="monotone" dataKey="avg7"   stroke="#6B21A8" strokeWidth={2.5} dot={false} isAnimationActive={false} />
...
function ScoreBreakdownChart({ scoreBreakdowns }) {
  ...
  const COLORS = { tir: '#22C55E', avg_glucose: '#6B21A8', variability: '#F59E0B', spikes: '#EF4444' }
...
function PrioritizedInsightsSection({ prioritized }) {
  ...
  const styles = {
    ...
    informational: { bg: '#F5F3FF', border: '#E9D5FF', text: '#6B21A8', hdr: '#E9D5FF', label: 'Informational' },
  }
...
function InsightTag({ type, text }) {
  const styles = {
    ...
    info:    { bg: '#F5F3FF', border: '#E9D5FF', text: '#6B21A8', icon: 'ℹ' },
  }
  ...
  return (
    <div>
      <div className="grid grid-cols-4 gap-2 mb-3">
        ...
        <StatBox label="Low"  value={overnightMeta.min_overnight}  unit=" mg/dL" color="#6B21A8" />
  ...
  return (
    <ResponsiveContainer width="100%" height={160}>
      <LineChart data={trendData} margin={{ top: 4, right: 8, bottom: 0, left: -20 }}>
        ...
        <Line type="monotone" dataKey="rollingAvg" stroke="#6B21A8" strokeWidth={2.5} dot={false} isAnimationActive={false} />
      </LineChart>
    </ResponsiveContainer>
  )
}
...
function ComparisonPanel() {
  ...
  return (
    <div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-slate-100">
              ...
              <th className="text-center py-2 px-3 text-plum-600 font-semibold">Stint 2</th>
      ...
      <div className="mb-5">
        <p className="text-[11px] text-slate-500 mb-2">Score trend — Stint 2 <span className="text-plum-600">vs</span> Stint 3 <span className="text-honey-500">(by day index)</span></p>
        <ResponsiveContainer width="100%" height={130}>
          <LineChart data={score_comparison} margin={{ top: 4, right: 8, bottom: 0, left: -20 }}>
            ...
            <Line type="monotone" dataKey="phase_2" stroke="#6B21A8" strokeWidth={2} dot={false} connectNulls isAnimationActive={false} />
      ...
      <div className="mb-5">
        ...
        <ResponsiveContainer width="100%" height={120}>
          <BarChart data={tod_comparison} margin={{ top: 4, right: 8, bottom: 0, left: -20 }}>
            ...
            <Bar dataKey="phase_2" fill="#6B21A8" radius={[4, 4, 0, 0]} barSize={20} isAnimationActive={false} />
        ...
        <div className="flex gap-2">
          <button onClick={() => setActiveTab('overview')}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
              activeTab === 'overview' ? 'bg-plum-600 text-white' : 'bg-white border border-slate-200 text-slate-600'
            }`}>
            Overview
          </button>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {displayDay.date === latestDay.date && (
                      <span className="text-[10px] bg-plum-100 text-plum-600 px-2 py-0.5 rounded-full">Latest</span>
                    )}
